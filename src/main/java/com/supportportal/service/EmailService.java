...
public class EmailService {
    ...
    private Message createEmail(String username, String password, String email) throws MessagingException {
        //The "javax.mail" library includes classes for defining email messages (such as MimeMessage)
        Message message = new MimeMessage(getEmailSession());
        //managing email addresses (such as InternetAddress),
        message.setFrom(new InternetAddress(FROM_EMAIL));
        message.setRecipients(TO, InternetAddress.parse(email, false));
        message.setRecipients(CC, InternetAddress.parse(CC_EMAIL, false));
        message.setSubject(EMAIL_SUBJECT);
        message.setText("Hello  your username is " + username + ", \n \n Your new account password is: " + password + "\n \n The Support Team");
        message.setSentDate(new Date());
        message.saveChanges();
        return message;
    }
    //Here we create our session
